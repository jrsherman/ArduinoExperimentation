...
void loop() {
  ...
  float y_mapped = map(distance, 20, 41, 0, 127);
  float y = distance ;
  ...
  //check how it effects the midi on off vals at a later date (make suer no conflicts ensue)
   if(y_mapped < 127){
     ...
     Serial1.write(176);
     Serial1.write(86);//select which parameter cc to work with
     //filterA
     ...
     dimAmount=map(y_mapped, 3, 127, 0, 4095);
